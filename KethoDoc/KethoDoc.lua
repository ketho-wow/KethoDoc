...
local filter_methods = {
	"AddCoalesceData", -- AlertFrameQueueMixin
	"AssembleMenuEntries", -- UnitPopupTopLevelMenuMixin
	"CreateMenuDescription", -- UnitPopupButtonBaseMixin
	"DoesFrameHaveEvent", -- CallbackRegistryMixin
	"GenerateHexColorMarkup", -- ColorMixin
	"GetAbilityID", -- PET_BATTLE_*
	"Init",
	"Initialize",
	"IsForbidden",
	"OnEvent",
...
local function FilterTableObject(name, value)
	if filter_table[name] then
		return true
	end
	for _, v in pairs(filter_names) do
		if name:find(v) then
			return true
		end
	end
	for _, method in pairs(filter_methods) do
...
function KethoDoc:GetTableObjects()
	...
	for name, value in pairs(_G) do
		if type(value) == "table" and not FilterTableObject(name, value) then
			for method, v2 in pairs(value) do
				if type(v2) == "function" then
					t[string.format("%s.%s", name, method)] = true
				end
			end
		end
	end
...
function KethoDoc:DumpTableObjects()
	local t  = self:GetTableObjects()
	local r = {}
	for k in pairs(t) do
		table.insert(r, k)
	end
	table.sort(r)
	eb:Show()
	eb:InsertLine("local TableObjects = {")
	local fs = '\t"%s",'
	for _, v in pairs(r) do
		eb:InsertLine(fs:format(v))
	end
	eb:InsertLine("}\n\nreturn TableObjects")
end
